...
class Register extends Component {
    ...
    submitAndSendRequest = async (e) => {
        e.preventDefault()
        await this.props.registerUser({fullName: this.state.fullName, password: this.state.password, phoneNumber: this.state.phoneNumber})
        ...
        return (
            <div className="register-container">
                ...
                <form method="POST" onSubmit={this.submitAndSendRequest} className="register-form">
                    ...
                    <div className="mb-3 passwordFieldContainer">
                        <input ref={passwordRef} className="input-field rpassword" type="password" minLength="8" maxLength="20" pattern="^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?!.*\s).*$" name="password" placeholder="password" onChange={this.setTheData} required /><br />
                    ...
                    <div>
                        <button type="submit" className="form-btn register white">Register</button>
                    </div>
                </form>
                <Footer />
            </div>
